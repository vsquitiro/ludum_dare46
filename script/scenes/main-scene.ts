...
export interface Plot extends AugmentedSprite {
    getPlotDef(): any;
}
export interface Spring extends AugmentedSprite {
    getSpringDef(): any;
}
export interface AugmentedZone extends Phaser.GameObjects.Zone {
    customProperties?: any;
    get(name: string): any;
    set(name: string, value: any): any;
    getTarget(): AugmentedSprite;
}

export interface AugmentedTiledObject extends Phaser.Types.Tilemaps.TiledObject {
    customProperties: any;
    get(name: string): any;
}
...
export type InteractionKeys = {
    accept: Phaser.Input.Keyboard.Key;
    accept2: Phaser.Input.Keyboard.Key;
...
class MainScene extends Phaser.Scene {
    ...
    nextRest: string;
    spriteIdx: number[];
    vatLevel0: Phaser.Tilemaps.DynamicTilemapLayer;
    vatLevel1: Phaser.Tilemaps.DynamicTilemapLayer;
    vatLevel2: Phaser.Tilemaps.DynamicTilemapLayer;
    crackAudio: Phaser.Sound.BaseSound;

    interactables: AugmentedSprite[];
    targetZones: AugmentedZone[];

    plots: Plot[];
    plotInteractions: AugmentedZone[];
    springs: Spring[];
    springInteractions: AugmentedZone[];
    foodTerminal: AugmentedSprite[];
    fuelTerminal: AugmentedSprite[];
    godControlInteractions: AugmentedZone[];
    ferts: AugmentedSprite[];
    fertInteractions: AugmentedZone[];
    target: Phaser.GameObjects.Rectangle;
    nearest: AugmentedSprite;
    player: Phaser.Physics.Arcade.Sprite;

    cursors: Phaser.Types.Input.Keyboard.CursorKeys;
    wasd: Phaser.Types.Input.Keyboard.CursorKeys;
    interaction: Partial<InteractionKeys>;
    ui: Partial<InteractionKeys>;
    // debug
    previousPadState: InputState;
    previousKeyState: InputState;
    ...
    create() {
        ...
        map.createStaticLayer('Floor Layer', tiles, 0,0);
        ...
        const transformObject = (obj: Phaser.Types.Tilemaps.TiledObject) => {
            const updatedObj:AugmentedTiledObject = { ...obj, customProperties: {}, get: null };
            updatedObj.customProperties = obj.properties.reduce((acc: any, prop: TiledObjectProp) => {
        ...
        const createZone = (interactor: AugmentedTiledObject) => {
            const x = interactor.x + (interactor.width / 2);
            const y = interactor.y + (interactor.height / 2);
            const zone = this.add.zone(x, y, interactor.width, interactor.height) as AugmentedZone;
        ...
        const createSprite = (sheet: string, frame:number) => (obj: AugmentedTiledObject) => {
            const x = obj.x + (obj.width / 2);
            const y = obj.y - (obj.height / 2);
            const sprite = this.physics.add.sprite(x, y, sheet, frame) as AugmentedSprite;
    ...
    handleMovementInput() {
        ...
        if (SystemState.allowMovement) {
            ...
            {
                (this.player.body as Phaser.Physics.Arcade.Body).setVelocityX(-playerVelocity);
                horizontalMove--;
            }
            ...
            {
                (this.player.body as Phaser.Physics.Arcade.Body).setVelocityY(-playerVelocity);
                verticalMove--;
            }
            ...
            if (gamepad) {
                ...
                if (Math.abs(stickPos.x) > .2) {
                    (this.player.body as Phaser.Physics.Arcade.Body).setVelocityX(stickPos.x * playerVelocity);
                    if(stickPos.x > 0) {
                        horizontalMove++;
                    } else {
                        horizontalMove--;
                ...
                if (Math.abs(stickPos.y) > .2) {
                    (this.player.body as Phaser.Physics.Arcade.Body).setVelocityY(stickPos.y * playerVelocity);
                    if(stickPos.y > 0) {
                        verticalMove++;
                    } else {
                        verticalMove--;
        ...
        if (SystemState.allowInteraction || SystemState.allowMessageInteraction) {
            ...
            if ((!this.previousKeyState.accept && curKeyState.accept) || (!this.previousPadState.accept && curPadState.accept)) {
                if (SystemState.message.current && SystemState.allowMessageInteraction) {
                    ...
                    switch (type) {
                    case 'plot':
                        this.interactWithPlot(this.nearest as Plot); break;
                    case 'spring':
                        this.interactWithSpring(this.nearest as Spring); break;
                    case 'foodTerminal':
    ...
    detectOverlap() {
        if (!this.player.body.touching.none || this.player.body.embedded) {
            const overlaps:AugmentedZone[] = [];
            this.physics.overlap(this.player, this.targetZones, (_, interactor: AugmentedZone) => overlaps.push(interactor));
            if (!overlaps.length) return;

            const mainOverlap = this.physics.closest(this.player, overlaps);

            const nearest = (mainOverlap as any).getTarget ? (mainOverlap as AugmentedZone).getTarget() : mainOverlap as AugmentedSprite;
    ...
    displayInteractAction(focus: AugmentedSprite) {
    ...
    interactWithSpring(spring: Spring) {
